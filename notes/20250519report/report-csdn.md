...
参考资料1: https://mp.weixin.qq.com/s/k\_cpKKIzjFDzjqKO29-5ow
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b62a2954842d4cc0b53e2b9d80907840.png)
...
2. ### [nanobot] Skill 核心加载流程

| **类型**          | **加载位置**      | **触发条件**                     |
| ------------------------- | ------------------------- | ---------------------------------------- |
| **Always Skills** | system\_prompt 完整内容 | metadata.always=true 且依赖满足        |
| 普通 Skills             | XML 摘要（元数据）      | Agent 通过 read\_file 按需加载完整内容 |
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/734a65adfc4b45c4beb787480ef14311.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5d4d5973ab9b481a9fff6b53b0cc312b.png)
...
| **优势** | **说明**                                               |
| ---------------- | -------------------------------------------------------------- |
| 语法简洁       | StringSchema("物料编号", min\_length=1) 比手写字典形式更简短 |
| IDE 支持       | 自带类型提示与参数联想，无需死记 JSON Schema 字段名          |
| 避免错误       | 规避 minLength / min\_length 这类命名格式混用写错问题        |
| 可复用         | Schema 实例支持多处复用、灵活组合拼装                        |
| 内置验证       | 提供 validate\_value() 方法，直接校验参数是否符合约束规则    |
...
4. ### tool\_parameters, tool\_parameters\_schema()
...
里面的`MessageBus`就是维护信息流的，解耦聊天渠道与Agent核心，代码非常简单。
...
* `nanobot/agent/loop.py`内的AgentLoop调用run，持续消费`inbound`中的内容。
  * 一个细节，如果content是"/stop"，则会停止，走`_handle_stop`。
* 如果content是"/stop"，则会停止，走`_handle_stop`。
* 而如果不是stop，则走`_dispatch`，此处是会拿到在`async with self._processing_lock`，调用` self._process_message(msg)`。
* 返回响应。渠道管理器或其他消费者通过 `bus.consume_outbound()` 获取响应，然后调用对应渠道的 `send()` 方法将消息发送到外部平台。继续以`nanobot/cli/commands.py`为例。
* 这块内容对python的协程`asyncio`有非常深入的应用，这块的调度基本就实现了Agent里面的异步、队列、随时打断、刷新的调度处理。
...
这是一个适配 Claude Code、Codex 等 Agent 环境的 Skill，用于将提示词生成单文件 HTML 横向翻页演示文稿，视觉风格是"编辑杂志 × 电子墨水"，就像 Monocle 杂志缝上了代码。 [GitHub](https://github.com/op7418/guizang-ppt-skill/blob/main/README.en.md)
...
参考资料1: [cc-switch设置在Claude code中使用GPT系列 - 小红书](https://www.xiaohongshu.com/explore/69d63551000000001a022b8c?xsec_token=ABKAGsezUhSFVN3ClXe9FarJ-A1SaN7A6SxEpLeQvg164=&xsec_source=pc_search&source=web_search_result_notes)

参考资料2: [【CC-Switch】:Claude Code 兼容 OpenAI 格式 API, Claude code 接入本地部署模型\_ccswitch的路由开关在哪-CSDN博客](https://blog.csdn.net/qq_45056135/article/details/160694324?spm=1001.2014.3001.5502)
...
| 功能特性                                          | 说明                                                                                 |
| --------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **一个应用, 六个 ​CLI**​**​ 工具** | 在单一界面中管理 Claude Code、Codex、Gemini CLI、OpenCode 和 OpenClaw， Hermes agent |
...
参考博客2: https://blog.csdn.net/qq\_45056135/article/details/160717854?spm=1001.2014.3001.5502

1. ### Claude Code Router 是什么
...
| **请求转发**     | 将 Claude Code 的请求转换为OpenAI兼容格式，转发到目标模型服务 |
...
* 多模型负载均衡和切换测试
* claude code router 配置不是热更新，每次完成之后需要利用`ccr restart` 重启才会生效

4. ### 快速使用
5. 安装claude code router
